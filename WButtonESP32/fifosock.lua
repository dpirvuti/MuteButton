...
local function wrap(sock)
  ...
  local ssend  = function(s,islast)
    ...
    if type(s) == "function" then
      if sslan ~= 0 then
        pcall(function () sock:send(ssla) end )
        ssla, sslan = nil, 0; gc()
    ...
    elseif type(s) == "string" and sslan < COALIMIT then
      ...
      if islast then
        ...
        if sslan < COALIMIT then
          pcall(function () sock:send(ssla .. concat(fsmall)) end)
          fsmall, lsmall = {}, 0
        else
          pcall(function () sock:send(ssla) end)
        end
    ...
    if s ~= nil then
      if sslan == 0 then
        pcall(function () sock:send(s) end)
      else
        pcall(function () sock:send(ssla .. s) end)
      end
      ssla, sslan = nil, 0; gc()
  ...
  return function(s)
    ...
    if corked then
      if #fbig <= COALIMIT
       then tmr.create():alarm(50, tmr.ALARM_SINGLE, sendnext)
       else sendnext()
      end
    end
  end
